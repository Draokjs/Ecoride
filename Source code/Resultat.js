...
document.getElementById("Tripresultdate").innerHTML = Array.from({ length: 5 }, (_, i) => {
  const date = new Date(Date.now() + i * 24 * 60 * 60 * 1000); // add i days to today
  return `<button class="btn btn-outline-info me-2">${date.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</button>`;
}).join('');
