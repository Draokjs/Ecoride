// Display cities of departure and arrival
document.getElementById("Valuedeparture").innerHTML = localStorage.getItem("departure") + " -> " + localStorage.getItem("arrival");

document.getElementById("Datedeparture").innerHTML = new Date(localStorage.getItem("dateDeparture")).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' }).replace(/^\w/, c => c.toUpperCase())
  + " , passagers: " + localStorage.getItem("numberOfPassengers");

// Generate 5 buttons with the next 5 days, starting from today
document.getElementById("Tripresultdate").innerHTML = Array.from({ length: 5 }, (_, i) => {
  const date = new Date(Date.parse(localStorage.getItem("dateDeparture")) + (i + 1) * 24 * 60 * 60 * 1000); // add i+1 days to today
...
// Show tripstart and tripend with random values
document.getElementById("Tripstart").innerHTML = 
  `<b>${`${Math.floor(Math.random() * 24)}`.padStart(2, '0')}h ${`${Math.floor(Math.random() * 60) + 2}`.padStart(2, '0')}</b>
...
document.getElementById("Tripend").innerHTML = 
  ...
  //  Generate random price and display coin logo
...
// Addition tirplength and write direct
